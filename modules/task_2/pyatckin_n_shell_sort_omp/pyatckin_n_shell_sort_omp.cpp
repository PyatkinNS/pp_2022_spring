...
#include "../../../modules/task_2/pyatckin_n_shell_sort_omp/pyatckin_n_shell_sort_omp.h"
#include <omp.h>
#include <ctime>
#include <random>
#include <vector>
...
std::vector <int> ShellSort(const std::vector <int>& vec) {
  ...
  for (step = size / 2; step > 0; step /= 2)
    for (i = step; i < size; i++)
      for (j = i - step; j >= 0 &&
          resulVec[j] > resulVec[j + step]; j -= step) {
        tmp = resulVec[j];
...
#pragma omp parallel shared(vec, remainder) private(localVec)
  {
    ...
    for (std::size_t i = 1; i < nthreads; i++) {
      if (tid == i) {
        std::copy(vec.begin() + delta * i + remainder,
            vec.end() - delta *(nthreads - i - 1), localVec.begin());
      }
    }
